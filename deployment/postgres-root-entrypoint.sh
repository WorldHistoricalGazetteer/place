...
docker_create_db_directories() {
        ...
        if [ -n "${POSTGRES_INITDB_WALDIR:-}" ]; then
                ...
                # REMOVED: chown commands that would fail with NFS root-squash
                chmod 700 "$POSTGRES_INITDB_WALDIR"
        fi

        # REMOVED: All chown commands for PGDATA and /var/run/postgresql
}
...
_main() {
        ...
        if [ "$1" = 'postgres' ] && ! _pg_want_help "$@"; then
                ...
                # This allows running as root throughout
